<script context="module">
  export async function preload() {
    try {
      ...
      // Page content
      const {
        fields: { pageHeading, body, gallery },
      } = content[0];
      // List Content
      const {
        fields: { listItems },
...
<Container>
  <div class="md:grid grid-cols-3 grid-rows-1 gap-20 mb-12">
    <div class="col-span-1">
      <div
        class="relative max-w-xs mx-auto mb-8 md:mb-0"
        style={`--aspect-ratio:${profileAsset.details.image.width / profileAsset.details.image.height}`}>
        <img
          class="z-1 transform scale-95"
          alt="Jesse Hoffman Profile"
